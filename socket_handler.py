...
def write_socket_status():
    ...
    try:
        ...
        status = {
            ...
            "last_prices": {
                k: {"ltp": v.get("ltp", 0)} for k, v in INSTRUMENT_LTP.items()
            },
        }
        ...
        try:
            if SOCKET_RECONNECT_EVENT.is_set():
                ...
                MARKET_FEED = marketfeed.DhanFeed(
                    client_id, access_token, instruments, "v2"
                )
...
def should_process_tick(now_ms: int) -> bool:
    return (now_ms - LAST_TICK_TIME.timestamp() * 1000) >= config.MIN_TICK_INTERVAL_MS
