...
class HeartbeatMonitor:
    ...
    def check_heartbeat(self) -> Tuple[bool, str]:
        ...
        return True, f"OK (age: {int(heartbeat.age_seconds())}s, cycles: {heartbeat.scanner_cycle})"
    ...
    def run_loop(self, auto_restart: bool = True) -> None:
        ...
        Args:
            auto_restart: Whether to attempt restart if unhealthy
        """
        logging.info(f"🔍 Heartbeat monitor started (check interval: {self.check_interval}s)")
...
def main():
    ...
    parser.add_argument("--status", "-s", action="store_true",
                       help="Show current heartbeat status")
    parser.add_argument("--stop", action="store_true",
                       help="Send stop signal to bot")
    parser.add_argument("--timeout", "-t", type=int, default=HEARTBEAT_TIMEOUT,
