    ...
    def load_cached_events(self) -> bool:
        ...
        try:
            if EVENTS_CACHE_FILE.exists():
                ...
                self.events = [EconomicEvent.from_dict(e) for e in data.get("events", [])]
                self.last_update = datetime.fromisoformat(data["last_update"]) if data.get("last_update") else None
                
                # Check if cache is still valid (less than 1 hour old)
                if self.last_update and (datetime.now() - self.last_update).total_seconds() < EVENTS_UPDATE_INTERVAL:
            ...
            if response.status_code == 200:
                ...
                for item in data:
                    try:
                        ...
                        event_date = datetime.strptime(item.get("date", ""), "%Y-%m-%dT%H:%M:%S%z")
                        ...
                        event = EconomicEvent(
                            ...
                            timestamp=event_date.replace(tzinfo=None),  # Convert to naive datetime
                            impact=impact,
    ...
    def add_manual_events(self) -> None:
        ...
        if manual_events_file.exists():
            try:
                ...
                logging.debug(f"Added {len(manual_data.get('events', []))} manual events")
            except Exception as e:
...
def start_calendar_updater(interval_seconds: int = EVENTS_UPDATE_INTERVAL) -> threading.Thread:
    """
    ...
    def updater():
        ...
        while True:
            try:
                calendar.update_events(force=True)
                logging.debug(f"Economic calendar updated: {len(calendar.events)} events")
            except Exception as e:
