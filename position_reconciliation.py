...
def fetch_broker_positions() -> Tuple[bool, List[BrokerPosition]]:
    ...
    try:
        ...
        if response.get('status') == 'failure':
            logging.error(f"Failed to fetch positions: {response.get('remarks', 'Unknown error')}")
            return False, []
...
def reconcile_positions(active_trade: Dict[str, Any]) -> ReconciliationResult:
    ...
    local_option_id = str(active_trade.get('option_id', '')) if local_has_position else None
    ...
    if local_has_position and not matching_broker_position:
        ...
        result.discrepancies.append(f"Local option_id {local_option_id} not found at broker")
        
        logging.error(f"🚨 RECONCILIATION MISMATCH: {result.message}")
        send_alert(f"🚨 **POSITION MISMATCH**\n{result.message}\n\nManual verification required!")
    ...
    if not local_has_position and broker_positions:
        ...
        if our_positions:
            ...
            result.discrepancies.append(f"Untracked positions: {[p.trading_symbol for p in our_positions]}")
            
            logging.warning(f"⚠️ RECONCILIATION WARNING: {result.message}")
            send_alert(f"⚠️ **UNTRACKED POSITION**\n{result.message}\n\nCheck if bot missed a trade entry!")
    ...
    if local_has_position and matching_broker_position:
        ...
        if matching_broker_position.quantity != local_lot_size:
            ...
            logging.info(f"✅ Position reconciled: {active_trade.get('instrument')} @ ₹{matching_broker_position.average_price}")
...
def auto_fix_mismatch(result: ReconciliationResult, active_trade: Dict[str, Any]) -> bool:
    """
    ...
    if result.status == ReconciliationStatus.MISMATCH_LOCAL_ONLY:
        ...
        logging.warning("⚠️ Auto-fixing: Clearing local state (position appears closed at broker)")
    ...
    if result.status == ReconciliationStatus.MISMATCH_DETAILS:
        ...
        if result.broker_positions:
            ...
            logging.warning(f"⚠️ Auto-fixing: Updating local quantity to match broker ({broker_pos.quantity})")
...
def run_periodic_reconciliation(active_trade: Dict[str, Any], interval_seconds: int = 300):
    """
        ...
        while True:
            try:
                ...
                if result.status != ReconciliationStatus.MATCHED:
                    ...
                    if result.status in [ReconciliationStatus.MISMATCH_LOCAL_ONLY, 
    ...
    thread = threading.Thread(target=reconcile_loop, daemon=True, name="ReconciliationThread")
    thread.start()
...
def verify_trade_entry(order_id: str, expected_security_id: str, expected_qty: int) -> Tuple[bool, str]:
    """
    ...
    for pos in positions:
        if pos.security_id == expected_security_id:
            if pos.quantity >= expected_qty:
                ...
                return False, f"Quantity mismatch: expected {expected_qty}, got {pos.quantity}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--positions", action="store_true", help="List broker positions")
    ...
    if args.positions:
        ...
        if success:
            ...
            for pos in positions:
                print(f"  {pos.trading_symbol}: {pos.position_type} {pos.quantity} @ ₹{pos.average_price:.2f}")
        else:
    ...
    if args.orders:
        ...
        if success:
            ...
            for order in orders:
                print(f"  {order.get('tradingSymbol')}: {order.get('transactionType')} {order.get('quantity')} - {order.get('orderStatus')}")
    ...
    if args.trades:
        ...
        if success:
            ...
            for trade in trades:
                print(f"  {trade.get('tradingSymbol')}: {trade.get('transactionType')} {trade.get('tradedQuantity')} @ ₹{trade.get('tradedPrice')}")
