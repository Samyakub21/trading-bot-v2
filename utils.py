...
# =============================================================================
def robust_api_call(
    max_retries: int = 3, base_delay: float = 1.0, backoff_factor: float = 2.0
):
    """
                ...
                try:
                    ...
                    if (
                        ...
                        if attempt < max_retries:
                            ...
                            logging.warning(
                                f"⚠️ Rate limit hit (429) for {func.__name__}. Waiting {wait_time}s before retry..."
                            )
                            time.sleep(wait_time)
                            continue
                        else:
                            logging.error(
                                f"❌ Rate limit persists after {max_retries} retries for {func.__name__}"
                            )
                            raise
                    else:
                    ...
                    if attempt < max_retries:
                        logging.warning(
                            f"⚠️ API call failed for {func.__name__} (attempt {attempt + 1}/{max_retries + 1}): {e}. Retrying in {delay:.1f}s..."
                        )
                        time.sleep(delay)
                        delay *= backoff_factor
                    else:
                        logging.error(
                            f"❌ API call failed permanently for {func.__name__} after {max_retries + 1} attempts"
                        )
...
def send_high_priority_alert(msg: str) -> None:
    ...
    try:
        ...
        if response.status_code != 200:
            logging.error(
                f"High-priority Telegram alert failed: {response.status_code}"
            )
    except requests.exceptions.Timeout:
...
def calculate_resistance_support_zones(
    df: pd.DataFrame, current_price: float, signal: str, lookback: int = 20
) -> Dict[str, List[float]]:
    """
...
def check_signal_cooldown(
    signal: str, last_signal: Optional[str], last_signal_time: Optional[datetime]
) -> Tuple[bool, str]:
