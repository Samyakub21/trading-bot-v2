...
def main():
    ...
    logging.info(
        f"  Trade History: {trade_history_file} - {'EXISTS' if os.path.exists(trade_history_file) else 'NOT FOUND'}"
    )
    logging.info(
        f"  Daily P&L: {daily_pnl_file} - {'EXISTS' if os.path.exists(daily_pnl_file) else 'NOT FOUND'}"
    )
    logging.info(
        f"  State: {state_file} - {'EXISTS' if os.path.exists(state_file) else 'NOT FOUND'}"
    )
    ...
    trades, pnl, state = db.migrate_from_json(
        trade_history_file, daily_pnl_file, state_file
    )
