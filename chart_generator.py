...
def generate_trade_chart(
    ...
    try:
        ...
        # Draw entry line
        ax_price.axhline(y=entry_price, color=CHART_STYLE['entry_color'], 
                         linestyle='--', linewidth=2, label=f'Entry: {entry_price}')
        
        # Draw stop loss line
        ax_price.axhline(y=stop_loss, color=CHART_STYLE['sl_color'], 
                         linestyle='--', linewidth=2, label=f'SL: {stop_loss}')
        
        # Draw target lines if provided
        if targets:
            for i, target in enumerate(targets[:3], 1):  # Max 3 targets
                ax_price.axhline(y=target, color=CHART_STYLE['target_color'], 
        ...
        fig.text(0.99, 0.01, f"Generated: {timestamp}", fontsize=8, 
...
def _plot_candlesticks(ax, df: pd.DataFrame) -> None:
    ...
    try:
        ...
        for i in range(len(df)):
            ...
            ax.plot([dates[i], dates[i]], [low_price, high_price], 
                    color=color, linewidth=1)
            
            # Draw body
            if body_height > 0:
                rect = Rectangle((dates[i] - width/2, body_bottom), 
                                  width, body_height,
                                  facecolor=color, edgecolor=color)
                ax.add_patch(rect)
            else:
                # Doji candle
                ax.plot([dates[i] - width/2, dates[i] + width/2], 
...
def _plot_volume(ax, df: pd.DataFrame) -> None:
    ...
    try:
        ...
        ax.bar(dates, df['volume'], width=0.0005, color=colors, 
        ...
        if not ema_cols:
            ...
            for period, color in zip([9, 21, 50], CHART_STYLE['ema_colors']):
                if len(df) >= period:
                    ...
                    ax.plot(df.index, ema, color=color, linewidth=1, 
                            alpha=0.7, label=f'EMA{period}')
        else:
            for i, col in enumerate(ema_cols[:3]):
                color = CHART_STYLE['ema_colors'][i % len(CHART_STYLE['ema_colors'])]
                ax.plot(df.index, df[col], color=color, linewidth=1, 
                        alpha=0.7, label=col.upper())
...
def _highlight_entry_candle(ax, df: pd.DataFrame, signal: str) -> None:
    ...
    try:
        ...
        ax.scatter([last_idx], [arrow_y], marker=arrow_marker, 
                   s=200, c=arrow_color, zorder=5, edgecolors='black')
        
    except Exception as e:
        logging.debug(f"Entry highlight error: {e}")


def _add_indicator_annotations(ax, df: pd.DataFrame, indicators: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if annotation_text:
            ...
            ax.text(x_max, y_max, text_str, fontsize=9, 
...
def send_chart_to_telegram(
    ...
    try:
        ...
        files = {
            'photo': ('trade_chart.png', image_bytes, 'image/png')
        }
