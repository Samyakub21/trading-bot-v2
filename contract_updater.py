...
def find_current_month_future(
    ...
    for contract in contracts:
        ...
        symbol_matches = trading_symbol.startswith(underlying_upper) and (
            len(trading_symbol) == len(underlying_upper)
            or not trading_symbol[len(underlying_upper)].isalpha()
        )
...
def get_updated_instrument_config(
    instrument_key: str, current_config: Dict[str, Any]
    ...
    logging.info(
        f"✅ Updated {instrument_key}: Future ID={security_id}, Expiry={expiry_date}, Lot={lot_size}"
    )
...
def save_contract_cache(contracts: Dict[str, Dict]):
    ...
    cache_data = {"updated_at": datetime.now().isoformat(), "contracts": contracts}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--download", action="store_true", help="Download fresh scrip master"
    )
    parser.add_argument(
        "--update", action="store_true", help="Update all instrument configs"
    )
    parser.add_argument(
        "--show", type=str, help="Show contract details for instrument (e.g., CRUDEOIL)"
    )
    parser.add_argument(
        "--expiries", type=str, help="Show next expiry dates for instrument"
    )
