...
AUTO_SQUARE_OFF_BUFFER = 5  # Minutes before market close to auto square-off (avoid broker penalty)

# SIGNAL STRENGTH CONFIG
RSI_BULLISH_THRESHOLD = 60  # RSI must be above this for bullish signal (stronger filter)
RSI_BEARISH_THRESHOLD = 40  # RSI must be below this for bearish signal (stronger filter)
...
def switch_active_instrument(instrument_key):
    ...
    required_fields = ["future_id", "lot_size", "strike_step", "expiry_date", 
                       "exchange_segment_str", "exchange_segment_int"]
    inst = INSTRUMENTS[instrument_key]
    for field in required_fields:
        if field not in inst or inst[field] is None:
            logging.error(f"Instrument {instrument_key} missing required field: {field}")
            return False
    ...
    logging.info(f"🔄 Switched active instrument to: {INSTRUMENT['name']} ({instrument_key})")
    return True
                ...
                if status in ['TRADED', 'FILLED']:
                    ...
                    if order_status and order_status.get('status') == 'success':
                        ...
                        if order_data.get('orderStatus') in ['TRADED', 'FILLED']:
                            avg_price = order_data.get('tradedPrice', 0)
                            return True, {"order_id": order_id, "avg_price": avg_price, "status": "FILLED"}
                    
                    # ⚠️ ZOMBIE ORDER FIX: Cancel the order if it's still pending
                    logging.warning(f"[{action}] Order not filled in time. Cancelling order {order_id} to prevent zombie trade")
                    try:
...
def get_resampled_data():
    try:
        ...
        if data['status'] == 'failure': return None, None
    ...
    while not SHUTDOWN_EVENT.is_set():
        try:
            ...
            if SOCKET_RECONNECT_EVENT.is_set():
                ...
                time.sleep(2)
                instruments = get_all_instrument_subscriptions()  # Refresh subscription list
                MARKET_FEED = marketfeed.DhanFeed(CLIENT_ID, ACCESS_TOKEN, instruments, version)
...
def get_dynamic_sl(action, df, buffer=2):
    try:
        ...
        if action == "BUY": # Bullish Trade SL
            ...
            if (current_cmp - sl_price) < 5: sl_price = current_cmp - 10
                
        else: # Bearish Trade SL ("SELL" Signal)
            swing_high = last_two['high'].max()
            sl_price = swing_high + buffer
            if (sl_price - current_cmp) < 5: sl_price = current_cmp + 10
...
def close_trade(exit_reason, exit_price_future, exit_price_option):
    ...
    trade_exchange_segment = active_trade.get("exchange_segment_str", EXCHANGE_SEGMENT_STR)
    ...
    while not SHUTDOWN_EVENT.is_set():
        ...
        if active_trade["status"]:
            ...
            if should_square_off:
                ...
                option_ltp = OPTION_LTP if OPTION_LTP > 0 else active_trade.get("option_entry", 0)
                ...
                exit_success, exit_details = verify_order(exit_response, "EXIT-AUTO-SQUAREOFF")
                exit_price = exit_details.get("avg_price", option_ltp) if exit_success else option_ltp
                
                close_trade(f"AUTO SQUARE-OFF ({square_off_msg})", LATEST_LTP, exit_price)
                time.sleep(1)
                continue
        ...
        if LATEST_LTP != 0 and active_trade["status"]:
            ...
            sl_hit = False
            if trade_type == "BUY":
                if ltp <= active_trade["sl"]: sl_hit = True
            ...
            if sl_hit:
                ...
                exit_limit_price = round(option_ltp * (1 - LIMIT_ORDER_BUFFER), 2)  # 1% buffer below LTP for sell
                ...
                if exit_success:
                    ...
                    exit_price = option_ltp  # Use last known LTP if order verification fails
            ...
            if current_r >= 4.0 and active_trade["step_level"] < 4:
                lock_r = 3.0; active_trade["step_level"] = 4; msg = "🚀 **Step 3 (1:4)**"
            elif current_r >= 3.0 and active_trade["step_level"] < 3:
                lock_r = 2.0; active_trade["step_level"] = 3; msg = "🚀 **Step 2 (1:3)**"
            elif current_r >= 2.0 and active_trade["step_level"] < 2:
                lock_r = 1.0; active_trade["step_level"] = 2; msg = "✅ **Step 1 (1:2)**"
...
if __name__ == "__main__":
    ...
    if MULTI_SCAN_ENABLED:
        ...
        logging.info(f"📊 Active Instrument: {INSTRUMENT['name']} ({ACTIVE_INSTRUMENT})")
    ...
    if MULTI_SCAN_ENABLED:
        ...
        for inst_key in get_instruments_to_scan():
            ...
            if inst["exchange_segment_str"] == "MCX" and not mcx_shown:
                logging.info(f"   MCX: {inst['market_start']} - {inst['market_end']} | No trades after {inst['no_new_trade_after']}")
                mcx_shown = True
            elif inst["exchange_segment_str"] == "NSE_FNO" and not nse_shown:
                logging.info(f"   NSE: {inst['market_start']} - {inst['market_end']} | No trades after {inst['no_new_trade_after']}")
                nse_shown = True
    else:
    ...
    logging.info(f"📈 Today's Stats: P&L: ₹{daily_stats['pnl']} | Trades: {daily_stats['trades']} | W:{daily_stats['wins']} L:{daily_stats['losses']}")
    logging.info("=" * 60)
    ...
    t1 = threading.Thread(target=start_socket); t1.start()
    time.sleep(5)
    t2 = threading.Thread(target=run_scanner, daemon=True); t2.start()
    t3 = threading.Thread(target=run_manager, daemon=True); t3.start()
        ...
        if active_trade.get("status"):
            ...
            if user_input == 'y':
                ...
                try:
                    ...
                    exit_success, exit_details = verify_order(exit_response, "EXIT-SHUTDOWN")
                    ...
                    if exit_success:
                        ...
                        logging.error("❌ Failed to close position. Manual intervention required!")
                        send_alert("🚨 **FAILED TO CLOSE POSITION ON SHUTDOWN** - Manual intervention required!")
                except Exception as e:
                    logging.error(f"Error closing position: {e}")
                    send_alert(f"🚨 **ERROR CLOSING POSITION**: {e}")
            else:
                logging.warning("⚠️ Position left open. State saved for recovery.")
                send_alert("⚠️ **BOT SHUTDOWN** - Position left open. State saved for recovery on restart.")
