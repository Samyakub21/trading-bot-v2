...
def validate_password(password: str) -> Tuple[bool, List[str]]:
    ...
    if len(password) < PASSWORD_POLICY["min_length"]:
        errors.append(
            f"❌ Password must be at least {PASSWORD_POLICY['min_length']} characters (got {len(password)})"
        )
    else:
    ...
    if PASSWORD_POLICY["require_special"]:
        ...
        if re.search(special_pattern, password):
            ...
            errors.append(
                f"❌ Must contain at least one special character: {PASSWORD_POLICY['special_chars']}"
            )
