...
class MockDhanClient:
    ...
    def get_fund_limits(self) -> Dict[str, Any]:
        ...
        return {
            "status": "success",
            "data": {"availabelBalance": 1000000, "utilizedAmount": 0},
        }
...
class BacktestEngine:
    ...
    def load_data(self, data: Optional[pd.DataFrame] = None) -> bool:
        ...
        if data is not None:
            ...
            self.historical_data = pd.read_csv(
                self.data_file, parse_dates=["time"], index_col="time"
            )
        else:
            # Try to load from Dhan API (for demo, create synthetic data)
            logging.warning(
                "No historical data provided. Using synthetic data for demo."
            )
    ...
    def _generate_synthetic_data(self) -> pd.DataFrame:
        ...
        df["high"] = df[["open", "close"]].max(axis=1) * (
            1 + np.random.uniform(0, 0.002, periods)
        )
    ...
    def run(self) -> BacktestResults:
        ...
        strategy = get_strategy(
            self.instrument, self.strategy_name, self.strategy_params
        )
        ...
        df_15 = (
            self.historical_data.resample("15min")
            .agg(
                {
                    "open": "first",
                    "high": "max",
        ...
        df_60 = (
            self.historical_data.resample("60min")
            .agg(
                {
                    "open": "first",
                    "high": "max",
    ...
    def _enter_trade(self, signal_info: Dict[str, Any], entry_time: datetime) -> None:
        ...
        logging.debug(
            f"📈 ENTRY: {signal} @ {price:.2f} | SL: {initial_sl:.2f} | Time: {entry_time}"
        )

    def _manage_trade(
        self, current_price: float, current_time: datetime, df_15: pd.DataFrame
    ) -> None:
    ...
    def _exit_trade(
        self, exit_price: float, exit_time: datetime, exit_reason: str
    ) -> None:
        ...
        logging.debug(
            f"{result_emoji} EXIT: {exit_reason} @ {exit_price:.2f} | "
...
def main():
    ...
    parser.add_argument(
        "--start", "-s", default="2025-01-01", help="Start date (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--end", "-e", default="2025-12-31", help="End date (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--strategy",
        "-st",
    ...
    parser.add_argument("--data", "-d", default=None, help="Path to CSV data file")
    parser.add_argument(
        "--output", "-o", default=None, help="Output file for results (JSON)"
    )
    parser.add_argument(
