"""
Generate Architecture Diagram for Trading Bot
"""
...
def draw_box(ax, x, y, w, h, label, sublabel="", color=COLORS['core'], fontsize=10):
    ...
    # Main label
    ax.text(x + w/2, y + h/2 + (0.8 if sublabel else 0), label, 
            ha='center', va='center', fontsize=fontsize, 
            fontweight='bold', color=color)
    
    # Sub label
    if sublabel:
        ax.text(x + w/2, y + h/2 - 1.2, sublabel, 
...
# Title
ax.text(50, 97, '🤖 TRADING BOT ARCHITECTURE', ha='center', va='center',
        fontsize=20, fontweight='bold', color=COLORS['main'])
ax.text(50, 94, 'Multi-Instrument Options Trading System', ha='center', va='center',
        fontsize=12, color=COLORS['subtext'])

# ================= MAIN ENTRY POINT =================
draw_box(ax, 43, 82, 14, 6, '🚀 Tradebot.py', 'Main Entry', COLORS['main'], 12)
...
ax.text(16, 59, 'Signal Detection\nEntry Triggers', ha='center', fontsize=7, color=COLORS['subtext'])
ax.text(36, 59, 'Trail Stop Loss\nP&L Calculation', ha='center', fontsize=7, color=COLORS['subtext'])
ax.text(57, 59, 'WebSocket Feed\nReal-time Data', ha='center', fontsize=7, color=COLORS['subtext'])
ax.text(78, 59, 'EMA Crossover\nRSI + MACD', ha='center', fontsize=7, color=COLORS['subtext'])

# ================= SUPPORT MODULES SECTION =================
draw_section(ax, 5, 32, 58, 18, '🛠️ SUPPORT MODULES', COLORS['support'])
...
ax.text(15, 34, 'Telegram Alerts\nState I/O', ha='center', fontsize=7, color=COLORS['subtext'])
ax.text(32, 34, 'Dead Man Switch\nMonitoring', ha='center', fontsize=7, color=COLORS['subtext'])
ax.text(51, 34, 'Sync with Broker\nVerify Positions', ha='center', fontsize=7, color=COLORS['subtext'])
...
ax.text(74, 34, 'Trade Charts\nTo Telegram', ha='center', fontsize=7, color=COLORS['subtext'])
ax.text(88, 34, 'Daily Summary\nEmail + HTML', ha='center', fontsize=7, color=COLORS['subtext'])

# ================= CONFIG SECTION =================
draw_section(ax, 5, 8, 45, 18, '⚙️ CONFIGURATION', COLORS['config'])

draw_box(ax, 8, 12, 12, 7, '⚙️ Config', 'config.py', COLORS['config'])
...
ax.text(61, 10.5, 'State\nLogs\nCache', ha='center', fontsize=7, color=COLORS['subtext'])
ax.text(74, 10.5, 'EOD Reports\nHTML Files', ha='center', fontsize=7, color=COLORS['subtext'])
ax.text(88, 10.5, 'Tools\nDocs\nBackup', ha='center', fontsize=7, color=COLORS['subtext'])

# ================= EXTERNAL SERVICES =================
ax.text(50, 3, '🌐 External: Dhan API • Telegram Bot • Email (SMTP) • Streamlit Dashboard',
        ha='center', fontsize=10, color=COLORS['external'])
...
for label, color in legend_items:
    ax.add_patch(plt.Rectangle((legend_x, legend_y - 0.5), 2, 1.5, 
...
# Save the figure
plt.tight_layout()
plt.savefig('architecture_diagram.png', dpi=150, facecolor='#1a1a2e', 
            edgecolor='none', bbox_inches='tight', pad_inches=0.5)
plt.savefig('Extras/data/architecture_diagram.png', dpi=150, facecolor='#1a1a2e', 
            edgecolor='none', bbox_inches='tight', pad_inches=0.5)
