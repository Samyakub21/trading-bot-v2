...
try:
    ...
    print(
        "Warning: bcrypt not installed. Using SHA256 fallback (less secure). Run: pip install bcrypt"
    )
...
if PROMETHEUS_AVAILABLE:
    ...
    try:
        ...
        PNL_GAUGE = Gauge(
            "trading_bot_daily_pnl", "Current daily profit/loss in INR", ["instrument"]
        )
        TOTAL_PNL_GAUGE = Gauge("trading_bot_total_pnl", "Total cumulative P&L")
        WIN_RATE_GAUGE = Gauge(
            "trading_bot_win_rate", "Current win rate percentage", ["instrument"]
        )
        ACTIVE_TRADE_GAUGE = Gauge(
            "trading_bot_active_trade", "Whether there is an active trade (1=yes, 0=no)"
        )
        ...
        WEBSOCKET_ERRORS = Counter(
            "trading_bot_websocket_errors_total", "Total WebSocket errors"
        )

        # Dashboard metrics
        DASHBOARD_LOGINS = Counter(
            "dashboard_logins_total", "Total dashboard login attempts", ["status"]
        )
            ...
            if submit:
                if not username or not password:
                    ...
                    if attempts_remaining > 0:
                        st.error(
                            f"❌ Invalid credentials. {attempts_remaining} attempt(s) remaining."
                        )
                    else:
                        st.error(
...
def render_websocket_status():
    ...
    with col1:
        if is_connected:
            st.metric(label="🌐 Connection", value="🟢 Connected", delta="Active")
        else:
            st.metric(
                label="🌐 Connection",
                value="🔴 Disconnected",
    ...
    with col3:
        ...
        st.metric(label="📨 Messages", value=f"{messages:,}")
...
def render_live_status():
    ...
    with col1:
        ...
        if is_active:
            st.metric(
                label="🤖 Bot Status", value="🟢 ACTIVE", delta=f"Trading {instrument}"
            )
        else:
            st.metric(
                label="🤖 Bot Status", value="💤 Scanning", delta="Awaiting signal"
            )
...
def render_performance_analytics():
    ...
    available_instruments = (
        df["instrument"].unique().tolist() if "instrument" in df.columns else []
    )

    col_filter1, col_filter2 = st.columns([1, 3])
    with col_filter1:
        filter_instrument = st.selectbox(
            "Filter by Instrument", ["All"] + available_instruments, index=0
        )
    ...
    with metric_cols[4]:
        avg_loss = (
            df_sorted[df_sorted["pnl"] <= 0]["pnl"].mean() if loss_count > 0 else 0
        )
...
def calculate_manual_sl_target(
    instrument: str, signal: str, entry_price: float
...
def render_manual_trade():
    ...
    **Manual Intervention Controls**  
    ...
    if state_data.get("status", False):
        st.warning(
            f"⚠️ There is already an active trade in {state_data.get('instrument', 'Unknown')}. "
        ...
        st.warning(
            "Use this only for emergency situations when you need to exit immediately."
        )

        if st.button(
            "🛑 Emergency Close Position", type="primary", use_container_width=True
        ):
            st.error(
                "⚠️ Emergency exit requested. Please confirm in the Tradebot terminal."
            )
            ...
            st.success(
                "Emergency exit signal sent to bot. Check bot terminal for confirmation."
            )
    ...
    with col1:
        ...
        st.caption(
            f"📊 **{inst_config.get('name', selected_instrument)}** | "
    ...
    if current_price > 0:
        ...
        with calc_col4:
            risk_per_lot = abs(current_price - stop_loss) * inst_config.get(
                "lot_size", 1
            )
            st.metric("Max Risk/Lot", f"₹{risk_per_lot:,.2f}", delta="per lot")
        ...
        with st.expander("🔧 Manual Override (Advanced)", expanded=False):
            ...
            with override_col1:
                manual_sl = st.number_input(
                    "Custom SL", min_value=0.0, value=float(stop_loss), step=0.5
                )

            with override_col2:
                manual_target1 = st.number_input(
                    "Custom Target 1", min_value=0.0, value=float(target_1), step=0.5
                )

            with override_col3:
                manual_target2 = st.number_input(
                    "Custom Target 2", min_value=0.0, value=float(target_2), step=0.5
                )
    ...
    confirm = st.checkbox(
        "I confirm this trade and understand the risks involved", value=False
    )
...
def render_settings_page():
    ...
    with tab1:
        ...
        daily_data = load_daily_pnl()
        remaining_loss = new_config.get("MAX_DAILY_LOSS", 5000) + daily_data.get(
            "pnl", 0
        )
        used_pct = (
            (1 - remaining_loss / new_config.get("MAX_DAILY_LOSS", 5000)) * 100
            if new_config.get("MAX_DAILY_LOSS", 5000) > 0
            else 0
        )
    ...
    with tab3:
        ...
        enabled_instruments = current_config.get(
            "ENABLED_INSTRUMENTS", list(INSTRUMENTS.keys())
        )
        instrument_priority = current_config.get(
            "INSTRUMENT_PRIORITY",
            {
                "CRUDEOIL": 1,
                # "GOLD": 2,
        ...
        st.caption(
            "Set priority for instrument selection when multiple signals occur (1 = highest priority)"
        )
        ...
        if new_enabled:
            ...
            st.warning(
                "⚠️ No instruments enabled. Enable at least one instrument above."
            )
        ...
        for inst_key in new_enabled:
            ...
            with st.expander(
                f"🔧 {inst_data.get('name', inst_key)} Settings", expanded=False
            ):
                ...
                with inst_col1:
                    ...
                    if use_custom:
                        custom_rsi_bull = st.slider(
                            ...
                            50,
                            80,
                            value=inst_settings.get(
                                "rsi_bullish",
                                strategy_params.get("rsi_bullish_threshold", 60),
                            ),
                        ...
                        custom_rsi_bear = st.slider(
                            ...
                            value=inst_settings.get(
                                "rsi_bearish",
                                strategy_params.get("rsi_bearish_threshold", 40),
                            ),
                    ...
                    if use_custom:
                        custom_vol_mult = st.slider(
                            ...
                            value=float(
                                inst_settings.get(
                                    "volume_multiplier",
                                    strategy_params.get("volume_multiplier", 1.2),
                                )
                            ),
                            step=0.1,
                            key=f"vol_{inst_key}",
                        ...
                        if inst_key in per_inst_settings:
                            if "PER_INSTRUMENT_SETTINGS" not in new_config:
                                new_config["PER_INSTRUMENT_SETTINGS"] = {}
                            new_config["PER_INSTRUMENT_SETTINGS"][inst_key] = {
                                "use_custom": False
                            }
                            config_changed = True
                ...
                if save_trading_config(new_config):
                    ...
                    log_entry = {
                        ...
                        "changes": {
                            k: new_config[k]
                            for k in new_config
                            if new_config[k] != current_config.get(k)
                        },
                    }
...
def main():
    ...
    if not st.session_state.panic_confirm:
        if st.sidebar.button(
            "🛑 EMERGENCY STOP ALL", type="primary", use_container_width=True
        ):
    ...
    if page == "📊 Dashboard":
        ...
        with col3:
            st.metric(
                "🔌 WebSocket", "Connected" if ws.get("connected") else "Disconnected"
            )
