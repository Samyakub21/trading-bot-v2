...
POLL_FALLBACK_THRESHOLD_SECONDS = int(
    config.get_trading_param("POLL_FALLBACK_THRESHOLD", 5)
)
...
def _get_ltp_via_rest(security_id: str, exchange_segment_str: str) -> Optional[float]:
    ...
    try:
        ...
        if quote_response.get("status") == "success":
            ...
            security_quote = quote_data.get(
                str(security_id), quote_data.get(int(security_id), {})
            )
...
def get_ltp_with_fallback(
    active_trade: Dict[str, Any], active_instrument: str
    ...
    trade_exchange_segment = active_trade.get(
        "exchange_segment_str", INSTRUMENTS[trade_instrument]["exchange_segment_str"]
    )
    ...
    if future_stale_seconds > POLL_FALLBACK_THRESHOLD_SECONDS:
        logging.warning(
            f"⚠️ Future data stale ({future_stale_seconds:.1f}s), triggering REST fallback"
        )
    ...
    if option_ltp > 0:
        ...
        if option_stale_seconds > POLL_FALLBACK_THRESHOLD_SECONDS:
            logging.warning(
                f"⚠️ Option data stale ({option_stale_seconds:.1f}s), triggering REST fallback"
            )

            # Get option LTP via REST
            option_id = active_trade.get("option_id")
            if option_id:
                rest_option_ltp = _get_ltp_via_rest(
                    str(option_id), trade_exchange_segment
                )
...
def close_trade(
    ...
    trade_lot_size = active_trade.get(
        "lot_size", INSTRUMENTS[trade_instrument]["lot_size"]
    )
    trade_exchange_segment = active_trade.get(
        "exchange_segment_str", INSTRUMENTS[trade_instrument]["exchange_segment_str"]
    )
    ...
    option_name = (
        f"{inst_name} {int(atm_strike)} {opt_type_str}"
        if atm_strike
        else f"{inst_name} {opt_type_str}"
    )
...
def run_manager(active_trade: Dict[str, Any], active_instrument: str) -> None:
    ...
    while not socket_handler.is_shutdown():
        ...
        latest_ltp, option_ltp, used_fallback = get_ltp_with_fallback(
            active_trade, active_instrument
        )
        last_tick_time = socket_handler.get_last_tick_time()
        last_option_tick_time = socket_handler.get_last_option_tick_time()
        ...
        if active_trade["status"] and option_ltp > 0:
            if (datetime.now() - last_option_tick_time).total_seconds() > 10:
                logging.warning(
                    "⚠️ OPTION DATA FEED LAG DETECTED - REST fallback active"
                )
        ...
        if active_trade["status"]:
            ...
            if should_square_off:
                ...
                if is_paper_trading:
                    logging.info(
                        f"📝 PAPER TRADING: Placing SQUARE-OFF EXIT order @ ₹{squareoff_limit_price}"
                    )
                    exit_response = {
                        "status": "success",
                        "data": {
                ...
                exit_success, exit_details = scanner.verify_order(
                ...
                close_trade(
                    f"AUTO SQUARE-OFF ({square_off_msg})",
                    latest_ltp,
                    exit_price,
                    active_trade,
                )
        ...
        if latest_ltp != 0 and active_trade["status"]:
            trade_lot_size = active_trade.get(
                "lot_size",
                INSTRUMENTS.get(
                    active_trade.get("instrument", active_instrument), {}
                ).get("lot_size", 10),
            )
            trade_exchange_segment = active_trade.get(
                "exchange_segment_str",
            ...
            if sl_hit:
                ...
                if is_paper_trading:
                    logging.info(
                        f"📝 PAPER TRADING: Placing SL EXIT order @ ₹{exit_limit_price}"
                    )
                    exit_response = {
                        "status": "success",
                        "data": {
                ...
                exit_success, exit_details = scanner.verify_order(
                    exit_response, "EXIT-SL", symbol_name=option_name
                )
            ...
            if current_r >= jackpot_threshold:
                ...
                if is_paper_trading:
                    logging.info(
                        f"📝 PAPER TRADING: Placing TARGET EXIT order @ ₹{target_exit_price}"
                    )
                    exit_response = {
                        "status": "success",
                        "data": {
...
def place_exit_order(active_trade: Dict[str, Any], exit_reason: str = "MANUAL") -> bool:
    ...
    # -------------------------------------------------------------------------
    is_critical = any(
        k in exit_reason for k in ["SHUTDOWN", "EMERGENCY", "User", "Manual"]
    )

    if is_critical:
        logging.info(
            f"🚨 CRITICAL EXIT ({exit_reason}): Forcing fresh price fetch via REST..."
        )
        try:
            trade_exchange_segment = active_trade.get(
                "exchange_segment_str", "MCX_COMM"
            )
            option_id = active_trade.get("option_id")
            ...
            if option_id:
                ...
                if quote_response.get("status") == "success":
                    ...
                    if sec_data:
                        fresh_ltp = float(
                            sec_data.get("last_price", sec_data.get("ltp", 0))
                        )
                        if fresh_ltp > 0:
                            logging.info(
                                f"✅ Fresh REST Price: {fresh_ltp} (Socket was: {option_ltp})"
                            )
    ...
    if not trade_exchange_segment:
        ...
        if inst_key and inst_key in INSTRUMENTS:
            trade_exchange_segment = INSTRUMENTS[inst_key].get(
                "exchange_segment_str", "MCX_COMM"
            )
        else:
    ...
    if is_paper_trading:
        logging.info(
            f"📝 PAPER TRADING: Placing EXIT order ({exit_reason}) @ ₹{exit_limit_price}"
        )
        exit_response = {
            "status": "success",
            "data": {
    ...
    exit_success, exit_details = scanner.verify_order(
        exit_response, f"EXIT-{exit_reason}", symbol_name=option_name
    )
